...
import { formatCurrency, getProductDisplayName, getProductSizeOptions, withBasePath } from "@/lib/utils";
    ...
    <article className="group overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="space-y-4 p-5">
        <div className="space-y-2">
          <p className="text-xs font-semibold tracking-[0.06em] text-slate-600">{product.brand}</p>
        ...
        <div className="flex flex-wrap gap-2 text-xs text-slate-600">
          <span className="rounded-full bg-slate-100 px-2.5 py-1">
            {sizeOptions.length > 1 ? `Tallas ${sizeOptions.join(" · ")}` : `Talla ${product.size}`}
          </span>
